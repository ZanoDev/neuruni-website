...
import { ActivityHero } from '@/heros/ActivityHero'
...
export async function generateStaticParams() {
  ...
  const params = activities.docs.map(({ slug }) => {
    return { slug }
  })
...
export default async function Activity({ params: paramsPromise }: Args) {
...
const queryActivityBySlug = cache(async ({ slug }: { slug: string }) => {
  ...
  const payload = await getPayload({ config: configPromise })
  console.log('queryActivityBySlug')
